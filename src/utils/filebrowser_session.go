...
import (
	...
	"github.com/linskybing/platform-go/src/k8sclient" // 假設這是你的 k8s client wrapper
...
func CreateFileBrowserPod(ctx context.Context, ns string, pvcName string) (*corev1.Pod, error) {
	...
	createdPod, err := k8sclient.Clientset.CoreV1().Pods(ns).Create(ctx, pod, metav1.CreateOptions{})
	if err != nil {
		if apierrors.IsAlreadyExists(err) {
			// If already exists, return current pod (or handle as success)
			return k8sclient.Clientset.CoreV1().Pods(ns).Get(ctx, podName, metav1.GetOptions{})
		}
...
func CreateFileBrowserService(ctx context.Context, ns string, pvcName string) (string, error) {
	...
	if err != nil {
		if apierrors.IsAlreadyExists(err) {
			...
			existingSvc, getErr := k8sclient.Clientset.CoreV1().Services(ns).Get(ctx, svcName, metav1.GetOptions{})
			if getErr != nil {
				return "", getErr
			}
...
func DeleteFileBrowserResources(ctx context.Context, ns string, pvcName string) error {
	...
	// 1. Delete Service
	errSvc := k8sclient.Clientset.CoreV1().Services(ns).Delete(ctx, svcName, metav1.DeleteOptions{})
	if errSvc != nil && !apierrors.IsNotFound(errSvc) {
		return fmt.Errorf("failed to delete service: %w", errSvc)
	}

	// 2. Delete Pod
	errPod := k8sclient.Clientset.CoreV1().Pods(ns).Delete(ctx, podName, metav1.DeleteOptions{
...
func StartUserHubBrowser(ctx context.Context, username string) (string, error) {
	...
	// k8sclient.Clientset.CoreV1().Pods(ns).Delete(ctx, appName, metav1.DeleteOptions{})
	// time.Sleep(1 * time.Second) // 稍微等一下

	_, err = k8sclient.Clientset.CoreV1().Pods(ns).Create(ctx, pod, metav1.CreateOptions{})
...
func StopUserHubBrowser(ctx context.Context, username string) error {
	...
	// 2. 刪除 Service
	err := k8sclient.Clientset.CoreV1().Services(ns).Delete(ctx, svcName, metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		return fmt.Errorf("failed to delete hub service: %w", err)
	}

	// 3. 刪除 Pod
	err = k8sclient.Clientset.CoreV1().Pods(ns).Delete(ctx, podName, metav1.DeleteOptions{
		GracePeriodSeconds: &gracePeriod,
	})
	if err != nil && !apierrors.IsNotFound(err) {
